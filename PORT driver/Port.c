...
void Port_Init(const Port_ConfigType* ConfigPtr)
{
    ...
    for(index = 0; index < PORT_CONFIGURED_PINS; index++)
    {
        ...
        /* Enable clock for PORT and allow time for clock*/
        ...
        if(Port_ConfigPtr->Pin[index].direction == OUTPUT)
        {
            ...
            if(Port_ConfigPtr->Pin[index].initial_value == PIN_LEVEL_HIGH)
            {
...
void Port_RefreshPortDirection(void)
{
    ...
    /* Check if the Driver is initialized before We use this function */
    if(Port_Status == PORT_NOT_INITIALIZED)
    {
